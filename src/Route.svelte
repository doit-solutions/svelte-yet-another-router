<script context="module">
    ...
    export const push = readable(function (href, state = null) {
        const completeHref = href.startsWith('/') ? `${get(pathBase)}${href}` : href;
...
<script>
    ...
    $: if (render) {
        ...
        queryParams = Array
            .from(p.keys())
            .reduce((c, k) => { let c2 = { ...c }; c2[k] = p.get(k); return c2; }, {});
        dispatch('match', { ...match, ...queryParams });
        if (onMatch !== undefined) { onMatch({ ...match, ...queryParams }); }
